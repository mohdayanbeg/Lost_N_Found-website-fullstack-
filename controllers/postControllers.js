...
exports.getAllPostsController=async(req,res)=>{
    try{
        const posts=await lfModel.find({})
        if(!posts){
...
exports.createPostController=async(req,res)=>{
    try{
        const{title,description,image,question,answer,type,user}=req.body
        //validatiion
        if(!title||!description||!image||!question||!answer||!type||!user){
        ...
        const newPost=new lfModel({title, description, image, question, answer, type, user});
...
exports.updatePostController=async(req,res)=>{
    try{
        ...
        const post=await lfModel.findByIdAndUpdate(id,{...req.body},{new:true})
        return res.status(200).send({
            success:true,
...
exports.getPostByIdController=async(req,res)=>{
    try{
        const{id}=req.params
        const post=await lfModel.findById(id)
        if(!post){
            return res.status(404).send({
                success:false,
...
exports.deletePostController=async(req,res)=>{
    try{
        const post=await lfModel.findOneAndDelete(req.params.id).populate("user")
